...
window.globalV6  = 'SOI-Revision: 1.536 $Revision: 1.536 $Date: 2013/11/20 11:11:06 $';
...
function soi_Tagwriter(platz, data)
{
	...
	if (is_video) {
		...
		if (mobilegate) {
			...
			if (needs_mobilegate) {
				...
				// prefetch = false;
				// FIXME: remove date test after 2013-11-25
				var now = new Date();
				if (window.SoiSetMid) window.SoiSetMid = now.getFullYear() == 2013 && now.getMonth() > 9 && now.getDate() > 24 ? 2 : 1;
			}
		}
	...
	if (special_showroom) {
		...
		var yieldprobe = SoiYieldProbe.get(platz) || ''; // yield and new adaudience
		var adprobe    = SoiAdProbe.get(platz) || ''; // new procter
		var ap_campaign_params = SoiAP.getCampaignParams(platz) || ''; // old procter and old adaudience
		...
		if (ap_campaign_params) all_collected.push(ap_campaign_params);
		if (adprobe) all_collected.push(adprobe);
...
window.SoiAP = {
	...
	init: function() {
		...
		if (window.SOI_VP && window.SOI_AUTOPLAY != 'off') {
			SoiLogger.logMsg('SoiAP.init: skipped because SOI_VP = ' + window.SOI_VP + ' in autoplay mode');
			return;
		}
		else if (!window.SOI_AP || typeof(window.SOI_AP) != 'object' || typeof(window.SOI_AP.length) != 'undefined') {
	...
	initRecommendations: function() {
		...
		SoiLogger.logMsg('SoiAP.initRecommendations called: ' + src);
	...
	doProfiling: function() {
		if (this.doProfiling.done) return;
		// dependancy between SoiAP.doProfiling and SoiYieldProbe.initKrux 
		if (window.SoiYieldProbe && SoiYieldProbe.initKrux && SoiYieldProbe.initKrux.done) return;
		...
		if (this.config.members[0] == 'adaudience') {
			...
			this.doProfiling.done = true;
			SoiLogger.logMsg('SoiAP.doProfiling: async loading of ' + src);
		}
	},
...
*/
window.SoiAdProbe = {
	// FIXME: remove after test period
	// force via yshowroom=pr*
	/*
		0 => disabled
		1 => init recommmendations, pass recommendations with xpr prefix
		2 => pass recommendations with pr prefix, and load campaigns
	*/
	testmode: 0,
	disabled:    true, // initially always disabled
	initialized: false,
	...
	init: function() {
		...
		if (window.SOI_VP && window.SOI_AUTOPLAY != 'off') {
			SoiLogger.logMsg('SoiAdProbe.init: skipped because SOI_VP = ' + window.SOI_VP + ' in autoplay mode');
			return;
		}
		else if (!window.SOI_AP || typeof(window.SOI_AP) != 'object' || typeof(window.SOI_AP.length) != 'undefined') {
			SoiLogger.logMsg('SoiAdProbe.init failed: SOI_AP not defined or invalid');
			return;
		}
		else if (window.SOI_AP.procter && !window.SOI_AP.procter.disabled && window.SOI_AP.procter.unit) {
			// FIXME: remove after test period - modify initial value only if aayp* param is present
			for (var i = 3; i >= 0; i--) {
				if (SoiGetShowroomParam('yshowroom', 'pr' + i)) {
					this.testmode = i;
					break;
				}
			}
		
			this.unit = window.SOI_AP.procter.unit;
			if (this.unit && window.SOI_PU1 || window.SOI_FB2 || window.SOI_SC1 || window.SOI_RT1)
				this.disabled = false;
		}

		SoiLogger.logMsg('SoiAdProbe.init: SoiAdProbe is ' + (this.disabled ? 'disabled' : 'enabled'));
		return this.disabled ? false : true;
	},
	initRecommendations: function() {
		// FIXME: remove after test period
		if ('testmode' in this && this.testmode < 1) return;

		if (this.disabled) return;

		window.asiPqTag = false;
		var src = 'http://pq-direct.revsci.net/pql?placementIdList=' + this.unit + '&cb=' + window.DFPOrd;
		document.write('<script type="text/javascript" src="' + src + '"><\/script>');
		SoiLogger.logMsg('SoiAdProbe.initRecommendations: ' + src);
	},
	store: function() {
		...
		if (window.asiPlacements) {
			...
			try {
				if (!asiPlacements[this.unit]) return;
				var data = asiPlacements[this.unit].data || {};
				var blob = asiPlacements[this.unit].blob;
				if (!data || !blob) return;
				
				// blob and key properties not needed for procter
				this.recommendations.blob = blob; 

				var campaign_ids = [];
				var prefix = 'PIPG-li'; // P&G prefix
				for (var k in data) {
					var item = data[k];
					if (!item || !item.key) continue;
					var found = String(k).match(new RegExp('^(' + prefix + ')([0-9]+)$'));
					if (!found) continue; // not P&G
					var id = Number(found[2]);
					campaign_ids.push(id); // k = PIPG-li188 => id = 188
				}
				var max = this.max_recommendations_per_unit;
				var selected = [];
				if (campaign_ids.length > max) {
					for (var j = 0; j < max; j++) {
						var index = parseInt(Math.random() * 10000000000) % campaign_ids.length;
						selected.push(campaign_ids[index]);
						campaign_ids.splice(index, 1);
					}
					campaign_ids = selected;
				}
				for (var i = 0; i < campaign_ids.length; i++) {
					var id = campaign_ids[i];
					var item = data[prefix + id];
					this.recommendations[id] = item.key;
				}
		...
		else if (friendly_iframe && parent.SoiAdProbe) {
	...
	sync: function(from, to) {
		...
		try {
			if (!from.SoiAdProbe || !to.SoiAdProbe) return;
			to.SoiAdProbe.disabled = from.SoiAdProbe.disabled;
			to.SoiAdProbe.session  = from.SoiAdProbe.session;
			to.SoiAdProbe.recommendations = {};
			to.SoiAdProbe.unit = '';
			if (to.SoiAdProbe.disabled) return;
			if (from.SoiAdProbe.unit) {
				to.SoiAdProbe.unit = from.SoiAdProbe.unit;
			}
			if (from.SoiAdProbe.recommendations) {
				for (var k in from.SoiAdProbe.recommendations) {
					to.SoiAdProbe.recommendations[k] = from.SoiAdProbe.recommendations[k];
				}
			}
		}
		catch(e) {}
	...
	get: function(ad_id) {
		// FIXME: remove condition after test period
		if ('testmode' in this && this.testmode < 1) return '';
		...
		var keyvalues = [];
		// FIXME: constant prefix 'pr' after test period
		var prefix = 'testmode' in this && this.testmode < 2 ? 'xpr' : 'pr';
		/* properties: blob, numeric ids */
		var recommendations = this.recommendations || {};
		for (var k in recommendations) {
			if (k == 'blob') continue;
			keyvalues.push(prefix + 'nn1=' + k);
		}
		return keyvalues.join(';') || '';
	},
	loadCampaign: function(config) {
		SoiLogger.logMsg('SoiAdProbe.loadCampaign should currently not be used.\nBook Procter campaigns via redirect template.');
		return;
		
		// FIXME: remove after test period
		if ('testmode' in this && this.testmode < 2) return;
		...
		var campaign_id = config.campaign_id;
		var unit = this.unit;
		if (!unit || !campaign_id) return;

		var adgroup = this.recommendations[campaign_id];
		var blob    = this.recommendations.blob;
		if (!blob || !adgroup) return;
		
		var src = 'http://imp.revsci.net/rtbads/pq?mode=s'
			+ '&placement=' + unit
			+ '&adgroup=' + adgroup
			+ '&blob=' + blob
			+ '&cachebuster=' + window.DFPOrd
			+ '&click=' + escape(config.clickredirect || '');
		document.write('<script type="text/javascript" src="' + src + '"><\/script>');
		SoiLogger.logMsg('SoiAdProbe.loadCampaign: ' + src);
	}
};
...
window.SoiYieldProbe = {
	// FIXME: remove after test period
	// force via yshowroom=aayp*
	/*
		0 => disabled
		1 => Krux
		2 => init recommmendations, pass recommendations with tt prefix
		3 => pass recommendations with aa prefix, and load campaigns
	*/
	testmode: 0,
	...
	init: function() {
		...
		if (window.SOI_VP && window.SOI_AUTOPLAY != 'off') {
		...
		else if (!window.SOI_YP || typeof(window.SOI_YP) != 'object' || typeof(window.SOI_YP.length) != 'undefined') {
			SoiLogger.logMsg('SoiYieldProbe.init failed: SOI_YP not defined or invalid');
		}
		else if (!window.SOI_YP.disabled && window.SOI_YP.units) {
			// FIXME: remove after test period - modify initial value only if aayp* param is present
			for (var i = 3; i >= 0; i--) {
				if (SoiGetShowroomParam('yshowroom', 'aayp' + i)) {
					this.testmode = i;
					break;
				}
			}
			...
			for (var k in units) {
				if (!units[k]) continue;
				if (k == 'nn1') {
					if ('testmode' in this && this.testmode < 2) continue;
					var accepted = this.getAcceptedFormats('list') || [];
					if (!accepted.length) continue;
					this.strf_formats = 'formats_' + units[k] + '=' + accepted.join(',');
				}
				else {
			...
			// force test campaigns: yshowroom=force&yshowroom=c1,c2,c111
			if (SoiGetShowroomParam('yshowroom', 'force')) {
				try {
					// FIXME: adjust expected pattern
					var found = top.location.search.match(/[?&#]yshowroom=(c[0-9c,]+[0-9])(?:[&#]|$)/);
					if (found) this.segs.push(found[1]);
				}
				catch(e) {}
			}

			SoiLogger.logMsg('SoiYieldProbe.init: SoiYieldProbe is ' + (this.disabled ? 'disabled' : 'enabled'));
		}
		return this.disabled ? false : true;
	},
	getAcceptedFormats: function(what, id) { // list|lookup, pu1|fb2|sc1|rt1|ma1
		// must be re-evaluated with each call since variables might have changed
		var permitted_formats = {
			104: window.SOI_PU1 && window.SOI_PL, // layer
			105: window.SOI_PU1 && window.SOI_PL, // banderole
			114: window.SOI_PU1 && window.SOI_PL, // interstitial
			115: window.SOI_PU1 && window.SOI_FA, // baseboard
			101: window.SOI_FB2 && window.SOI_WP, // wallpaper
			102: window.SOI_FB2 && window.SOI_WP, // wallpaper with bgcolor
			103: false && window.SOI_FB2 && window.SOI_WP, // wallpaper with bgcolor, clickable
			110: window.SOI_FB2 && window.SOI_WP && window.SOI_PL, // expandable wallpaper
			112: window.SOI_FB2 && window.SOI_PL, // expandable fullbanner2
			109: window.SOI_FB2 && window.SOI_BB, // billboard
			106: window.SOI_SC1 && window.SOI_PL, // skyscraper + layer
			111: window.SOI_SC1 && window.SOI_PL, // expandable skyscraper1
			113: window.SOI_RT1 && window.SOI_HP && window.SOI_PL, // halfpage + layer
			116: window.SOI_RT1 && window.SOI_PL, // expandable rectangle1
			107: window.SOI_RT1 && window.SOI_HP, // halfpage
			108: window.SOI_MA1 || window.SOI_TS1 // maxi ad
		};

		var formats = window.SOI_YP && !window.SOI_YP.disabled ? window.SOI_YP.formats || {} : {};
		this.formats = {};
		for (var k in formats) {
			if (permitted_formats[formats[k]]) {
				this.formats[k] = formats[k];
				if (k == 101) this.formats[102] = this.formats[102] = true; // different wallpaper format ids
			}
		}

		if (id == 'ts1') id = 'ma1';
		var retval = what == 'list' ? [] : {};
		for (var k in this.formats) {
			if (!formats[k] || (id && !k.match(new RegExp('^' + id)))) continue;
			what == 'lookup' ? retval[formats[k]] = true : retval.push(formats[k]);
		}
		if (what == 'list' && retval.length) retval = retval.sort();
		return retval; // array or object
	},
	initRecommendations: function() {
		this.init();
		if (this.disabled) return;
		...
		var strf_units = '';
		for (var k in this.units) strf_units += this.units[k] + ',';
		strf_units = strf_units.replace(/,+$/, '');
		if (!strf_units) return;

		var src = 'http://ad.yieldlab.net/yp/' + strf_units + '?'
			+ (this.strf_formats ? this.strf_formats + '&' : '')
			+ (this.segs && this.segs.length ? 'segs=' + this.segs.join(',') + '&' : '')
			+ 'ts=' + window.DFPOrd;
		document.write('<script type="text/javascript" src="' + src + '"><\/script>');
		SoiLogger.logMsg('SoiYieldProbe.initRecommendations: ' + src);
	},
	store: function() {
		// always - regardless of disabled state
		var friendly_iframe = SoiUtils.getIFrameRelation() == 1;
		if (window.yl) {
			...
			try {
				for (var k in this.units) {
					...
					if (!item) continue;
					// FIXME: remove condition after test period
					if (item.advertiser == 'adaudience' && 'testmode' in this && this.testmode < 2) continue;
					...
					if (k == 'nn1') {
						...
						for (var v in this.formats) {
							if (this.formats[v] == format) {
								this.recommendations[k].soi_key = v;
								break;
							}
						}
					}
	...
	get: function(ad_id) {
		...
		var accepted = this.getAcceptedFormats('lookup', id) || {}; // object
		var keyvalues = [];
		try {
			// FIXME: constant prefix 'aa' after test period
			var prefix = 'testmode' in this && this.testmode < 3 ? 'tt' : 'aa';
			/* properties: id, format, price, advertiser, curl */
			var result = this.recommendations[id];
			if (result) {
				// FIXME: maybe not be necessary if exclusion is handled by YieldProbe
				var critical_exclusions = String(window.SOI_EXCL).match(/(?:^|,)(fi|ga)(?:,|$)/) ? true : false;
				var format     = result.format || '';
				var advertiser = result.advertiser || '';
				if (advertiser == 'adaudience') {
					if (format) {
						 if (accepted[format]) keyvalues.push(prefix + id + '=1');
					}
					else {
						var prio = result.prio || 1;
						var is_low = prio > 7 ? true : false; // FIXME TBD: 12 low TKP, 7 normal
						if (!is_low || !critical_exclusions) // exclude low prio in case of critical exclusions
							keyvalues.push(prefix + id + (is_low ? 'low' : '') + '=1');
					}
				}
				else {
					keyvalues.push('yl=1'); // FIXME: exclude in case of critical exclusions
				}
			}
			result = this.recommendations.nn1;
			if (result && result.advertiser == 'adaudience'
				&& String(result.soi_key || '').match(new RegExp('^' + id)))
					keyvalues.push(prefix + result.soi_key + '=1');
		}
		catch(e) {}
		return keyvalues.join(';');
	},
	loadCampaign: function(config) {
		...
		var unit_id;
		var size;
		if (config.campaign == 'yl' || config.campaign == id && this.units[id]) {
			unit_id = this.units[id];
			size = config.size == '468x60' ? '728x90' : config.size;
		}
		else {
			// pu1, ma1, sowefos
			unit_id = this.units.nn1;
			size = '1x1';
		}
		if (!unit_id || !size) return;

		var recommendation = this.recommendations[id] || {};
		if (recommendation.id != unit_id) recommendation = this.recommendations.nn1 || {};
		var advertiser = recommendation.advertiser || ''
		var format     = recommendation.format || '';
		// FIXME: remove after test period
		if (advertiser == 'adaudience' && 'testmode' in this && this.testmode < 3) return;
		SoiAdTemplate.stored[ad_id] = String(advertiser + ' ' + format).replace(/^ +/, ''); // for adtrace

		if (advertiser == 'adaudience') window.ada_pubkey = '71i_' + (window.SOI_ADA || 'default');
		var publisher_id = 5710;
		var src = 'http://ad.yieldlab.net/d/' + [unit_id, publisher_id, size].join('/') + '?ts=' + window.DFPOrd;
		document.write('<script type="text/javascript" src="' + src + '"><\/script>');
		SoiLogger.logMsg('SoiYieldProbe.loadCampaign: ' + src);
	},
	initKrux: function() {
		// FIXME: remove after test period
		if ('testmode' in this && this.testmode < 1) return;

		if (this.initKrux.done) return;
		// dependancy between SoiAP.doProfiling and SoiYieldProbe.initKrux 
		if (window.SoiAP && SoiAP.doProfiling && SoiAP.doProfiling.done) return;

		this.init();
		if (this.disabled) return;

		function f() {
			// Krux Control Tag for "SOM"
			// Source: /snippet/controltag?confid=Ip_ckp5P&site=SOM&edit=1&raw=1
			...
			k.src = /^https?:\/\/([^\/]+\.)?krxd\.net(:\d{1,5})?\//i.test(src) ?
				...
				: src === "disable" ?
					  ""
					: (location.protocol === "https:" ? "https:" : "http:") + "//cdn.krxd.net/controltag?confid=Ip_ckp5P";
			if (k.src) {
				var s = document.getElementsByTagName('script')[0];
				s.parentNode.insertBefore(k, s);
				SoiLogger.logMsg('SoiYieldProbe.initKrux: async loading of ' + k.src);
			}
		}
		// FIXME: remove eventually
		if (SoiGetShowroomParam('yshowroom', 'kruxonload')) {
			SoiUtils.addEventHandler(window, 'load', f);
		}
		else {
			f();
		}

		this.initKrux.done = true;
	},
	getKruxData: function() {
		var data = {};
		if (window.Krux) {
			var prefix = 'kxadaudience_';
			var keys = ['user', 'segs'];
			for (var i = 0; i < keys.length; i++) {
				var key = keys[i];
				if (window.localStorage) {
					data[key] = window.localStorage[prefix + key];
				}
				else if (window.navigator.cookieEnabled) {
					var found = document.cookie.match(prefix + key + '=([^;]*)');
					data[key] = found ? unescape(found[1]) : '';
				}
			}
			if (data.segs) data.segs = data.segs ? data.segs.split(/,/) : [];
		}
		return data;
	}
};
...
window.SoiPrivacyInfo = {
		...
		initGlobalPrivacyInfo: function() {
				...
				try {
					...
					if (window.SOI_AP) {
						if (window.SOI_AP.adaudience && !window.SOI_AP.adaudience.disabled)
							items.push(
								{
						...
						if (window.SOI_AP.procter && !window.SOI_AP.procter.disabled)
							items.push(
								{
						...
						if (false && 'FIXME' && window.SOI_AP.procter && window.SOI_AP.procter.unit && !window.SOI_AP.procter.disabled)
							items.push(
								{
									title:    'Procter & Gamble',
									url:      'http://www.pg.com/en_US/privacy/german/cookie_notice.shtml',
									text:     text,
									linkText: linktext,
									usePopup: use_popup
								}
...
if (window.DFPSite && window.DFPZone) {
	SoiInitMoreVars();
	SoiYieldProbe.init(); // always!
	SoiAP.init(); // always!
	SoiAdProbe.init(); // always!
	...
	if (!window.SOI_INIT_DONE) {
		SoiNugg.init();
		try {
			SoiYieldProbe.initRecommendations(); // retargeting yield, new adaudience
			SoiYieldProbe.initKrux();
		}
		catch(e) {}
		try {
			SoiAP.doProfiling(); // only happening if Krux was skipped
			SoiAP.initRecommendations(); // old procter, old adaudience)
		}
		catch(e) {}
		try {
			SoiAdProbe.initRecommendations(); // new procter
		}
		catch(e) {}
		SoiRogator.init();
	}
